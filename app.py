...
def prepare_model_input(player_data, team_order_gold, team_chaos_gold):
    # Example: Create input vector based on player_data and calculated gold
    ...
    team_chaos_cs = sum(p["scores"].get("creepScore", 0) for p in player_data if p["team"] == "ORDER")
    team_chaos_kda = team_chaos_kills / (
        team_chaos_deaths if team_chaos_deaths > 0 else 1)
...
def predict_win_probability(model_input):
    ...
    return {
        "team_order_win": float(probs[0][1].item() * 100),  # Convert to percentage
        "team_chaos_win": float(probs[0][0].item() * 100)   # Convert to percentage
    }
